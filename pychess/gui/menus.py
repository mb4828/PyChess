...
class StartMenu:
    def __init__(self, on_start_press, on_quit_press):
        self.menu = pygame_menu.Menu(
            '', 300, 250, theme=MENU_THEME, mouse_motion_selection=True)
        self.menu.add.image(get_resource_path(
            constants.PATH_LOGO), scale=(.6, .6), scale_smooth=True)
    ...
    def draw(self, window, events):
        ...
        draw_solid_rect(window, constants.WINDOW_WIDTH,
                        constants.WINDOW_HEIGHT, 0, 0, (0, 0, 0), 85)
...
class PromotionMenu:
    def __init__(self, on_select):
        self.menu = pygame_menu.Menu(
            '', 300, 300, theme=MENU_THEME, mouse_motion_selection=True)
