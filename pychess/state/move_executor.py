...
def execute_move(
    ...
    """
    game_state.clear_square(start_x, start_y)
    game_state.set_piece(end_x, end_y, piece_code)

    # Castling: king moved 2 squares horizontally, so also reposition the rook
    if piece_code.startswith('k') and abs(end_y - start_y) == 2:
        if end_y == 6:  # kingside
            game_state.set_piece(end_x, 5, game_state.get_piece(end_x, 7))
            game_state.clear_square(end_x, 7)
        elif end_y == 2:  # queenside
            game_state.set_piece(end_x, 3, game_state.get_piece(end_x, 0))
            game_state.clear_square(end_x, 0)

    # En passant: captured pawn sits on the starting row at the destination column
    if is_en_passant:
        game_state.clear_square(start_x, end_y)
