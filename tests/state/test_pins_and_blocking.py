...
class TestCheckmateWithPinnedDefender:
    ...
    def test_checkmate_pinned_piece_cannot_block(self):
        ...
        board = empty_board()
        board.set_piece(7, 4, 'kl')
        board.set_piece(6, 4, 'rl')  # rook pinned along file by enemy rook
        board.set_piece(0, 4, 'rd')  # enemy rook pinning our rook
        board.set_piece(7, 0, 'qd')  # enemy queen giving check along rank
...
class TestIsInCheckmateOriginBug:
    ...
    def test_checkmate_missed_due_to_ghost_rook(self):
        ...
        board = empty_board()
        board.set_piece(7, 4, 'kl')  # king
        board.set_piece(6, 3, 'pl')  # pawns surrounding king
        board.set_piece(6, 4, 'pl')
        board.set_piece(6, 5, 'pl')
        board.set_piece(5, 4, 'rl')  # rook blocking enemy rook
        board.set_piece(0, 4, 'rd')  # enemy rook pinning our rook
        board.set_piece(7, 0, 'qd')  # enemy queen giving check along rank 7
        board.set_piece(7, 7, 'rd')  # enemy rook controlling other side of rank 7
        assert is_in_check(board, 'l') is True
